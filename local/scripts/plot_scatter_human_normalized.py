...
print('Running plot_scatter_human_normalized.py ...')
...
in_kmer_file = 'data/kmer_counts.txt'
out_file_pattern = 'plots/psi_vs_human_scatter_normalized_%d.png'
...
# Read in kmer counts
kmer_df = pd.read_csv(in_kmer_file, sep='\t')
kmer_seqs = [kmer.replace('T','U') for kmer in kmer_df['kmer']]
kmer_df['ss'] = kmer_seqs
kmer_df.set_index('ss', inplace=True)
del kmer_df['kmer']
kmer_df.rename(columns={'count':'kmer_ct'}, inplace=True)
...
# Merge kmer counts into all_df
all_df = all_df.merge(kmer_df,left_index=True, right_index=True, how='left')
...
for threshold in [20, 50]:
    ...
    for i, xcol in enumerate(xcols):
        ...
        ys = all_df[ycol][indices]/all_df[kmer_col][indices]
        ...
        # Make scatter plot
        color = 'slategrey'
        ys_jitter = ys #+ 0.3E-4*(np.random.rand(len(ys)) - .5) 
        ...
        ax.set_ylim(ylim)
        if i == 0:
            ax.set_ylabel('no. splice sites / no. genomic sites', fontsize=fontsize)
        else:
            ax.set_yticklabels([])
        ax.axhline(.1E-4, linestyle=':', color='blue')
